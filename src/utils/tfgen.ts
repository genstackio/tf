...
export const tfgen = async (configFile: string, sourceDir: string, targetDir: string) => {
    ...
    const config: { environments?: Record<string, string>; layers?: Record<string, unknown> } = await import (resolve(configFile));
    await Promise.all(Object.keys(config.environments || {}).map(async (name: string) => {
